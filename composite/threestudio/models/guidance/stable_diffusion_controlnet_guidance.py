...
from diffusers import DDIMScheduler, DDPMScheduler, StableDiffusionPipeline, ControlNetModel, StableDiffusionControlNetPipeline, AutoencoderKL
...
class StableDiffusionGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.vae_key is not None:
            vae = AutoencoderKL.from_pretrained(self.cfg.vae_key, cache_dir='/root/autodl-tmp/huggingface/hub').to(self.weights_dtype)
            pipe_kwargs["vae"] = vae
    ...
    def compute_grad_anpg(
        ...
        with torch.no_grad():
            ...
            control_image_input = torch.cat([control_image] * 3, dim=0)
