...
class Skeleton:
    ...
    def __init__(self, humansd_style=False, apose=False, playing=False):
        ...
        self.vertices = None
        self.faces = None
        self.motion_source = '/root/autodl-tmp/HumanGaussian/content/skiing.npz'
        
        if self.playing:
            # motion data
            self.motion_seq = np.load(self.motion_source)["poses"][:, 1:22]
            self.motion_seq = np.load(self.motion_source)["poses"][:, 0:21]
            self.Rh_seq = np.load(self.motion_source)["Rh"]
            self.Th_seq = np.load(self.motion_source)["Th"]
    ...
    def load_smplx(self, path, betas=None, expression=None, gender='neutral', seq_id=0):
        ...
        if self.playing:
            body_pose = np.array(self.motion_seq[seq_id % len(self.motion_seq)])
            body_orient = np.array(self.Rh_seq[seq_id % len(self.motion_seq)])
