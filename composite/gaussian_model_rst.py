...
from scene.deformation2 import deform_network
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self.rotation_param = nn.Parameter(torch.normal(mean=torch.tensor([0.5, 0.5, 0.5, 0.5]), std=0.1), requires_grad=True)
        self.scale_param = nn.Parameter(torch.normal(mean=1., std=0.3,size=(1,)), requires_grad=True)
        self.translate_param = nn.Parameter(torch.zeros(3), requires_grad=True)
    ...
    def get_deformed_everything(self, time, t_vertices=None, face_vertices=None, pose_params=None):
        ...
        means3D_deform, scales_deform, rotations_deform, opacity_deform = self._deformation(means3D[deformation_point], scales[deformation_point], 
                                                    ...
                                                    time[deformation_point], t_vertices, face_vertices, pose_params)
    ...
    @torch.no_grad()
    def extract_fields_t(self, resolution=128, num_blocks=16, relax_ratio=1.5, t=0, t_vertices=None, face_vertices=None, pose_params=None):
        # resolution: resolution of field
        ...
        xyzs, rotation, scale, opacities = self.get_deformed_everything(t, t_vertices, face_vertices, pose_params)
    ...
    def extract_mesh_t(self, path, density_thresh=1, t=0, resolution=128, decimate_target=1e5, t_vertices=None, face_vertices=None, pose_params=None):

        os.makedirs(os.path.dirname(path), exist_ok=True)

        occ = self.extract_fields_t(resolution, t=t, t_vertices=t_vertices, face_vertices=face_vertices, pose_params=pose_params).detach().cpu().numpy()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': list([self.scale_param]), 'lr': 0.005, "name": "scale_param"},
            {'params': list([self.rotation_param]), 'lr': 0.005, "name": "rotation_param"},
            {'params': list([self.translate_param]), 'lr': 0.005, "name": "translation_param"},
        ]
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if  "grid" in param_group["name"]:
                ...
                param_group['lr'] = lr
                # return lr
            elif param_group["name"] == "translation_param":
                param_group["lr"] = param_group["lr"] * ((0.5) ** (1.0 / 1200.0))
            elif param_group["name"] == "scale_param":
                param_group["lr"] = param_group["lr"] * ((0.5) ** (1.0 / 1200.0))
            elif param_group["name"] == "rotation_param":
                param_group["lr"] = param_group["lr"] * ((0.5) ** (1.0 / 1200.0))
    ...
    def save_ply(self, path, item=False):
        ...
        if item:
            xyz, rotation, scale = self.transform_gaussian()
    ...
    def quaternion_conjugate_torch(self,
        q: torch.Tensor,  # (batch_size, 4), (x, y, z, w)
    ):
        return torch.cat([-q[..., 0:3], q[..., 3:4]], dim=-1)

    def quaternion_multiply_torch(self,
    ...
    def quaternion_rotate_torch(self,
        q: torch.Tensor,  # (batch_size, 4)
        v: torch.Tensor,  # (batch_size, 3)
    ):
        # q = q / torch.norm(q, dim=-1, keepdim=True)
        v = torch.cat([v, torch.zeros_like(v[..., :1])], dim=-1)
        q_conj = self.quaternion_conjugate_torch(q / torch.norm(q, dim=-1, keepdim=True))
        return self.quaternion_multiply_torch(
            self.quaternion_multiply_torch(q / torch.norm(q, dim=-1, keepdim=True), v), q_conj)[..., :3]

    def rotate_rotation(self,
        q1: torch.Tensor,  # (batch_size, 4)
        q2: torch.Tensor,  # (batch_size, 4)
                           ):
        newq=self.quaternion_multiply_torch(q1,q2)
        return newq / torch.norm(newq, dim=-1, keepdim=True)
    
    def rescale(self, scale_param: float,_xyz,_scaling):

        return _xyz * scale_param, _scaling + torch.log(scale_param)
    
    def complete_rotation(self,xyz,rotation,rotation_param):
        
        return self.quaternion_rotate_torch(rotation_param, xyz), self.rotate_rotation(rotation_param, rotation)
    
    def translation(self, translate_vector,_xyz):
        
        return _xyz + translate_vector
        
    def transform_gaussian(self):
        
        _xyz, _rotation, _scaling=self._xyz, self._rotation, self._scaling
        _xyz, _scaling = self.rescale(self.scale_param.cuda(), _xyz, _scaling)
        _xyz, _rotation = self.complete_rotation(_xyz, _rotation, self.rotation_param.cuda())
        _xyz = self.translation(self.translate_param.cuda(), _xyz)
            
        return _xyz, _rotation, _scaling
    
    def transform_model(self, xyz, rotation, scaling):
        
        xyz = self.translation(self.translate_param.cuda(), xyz)
