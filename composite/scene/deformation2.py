...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, skips=[], args=None, use_res=False):
        ...
        if not self.use_res:
            self.pos_deform, self.scales_deform, self.rotations_deform, self.opacity_deform, self.lbs_deform = self.create_net()
        else:
            self.pos_deform, self.scales_deform, self.rotations_deform, self.opacity_deform, self.lbs_deform = self.create_res_net()
        self.args = args
    ...
    def create_net(self):
        ...
        return  \
            ...
            nn.Sequential(nn.ReLU(),nn.Linear(self.W,self.W),nn.ReLU(),nn.Linear(self.W, 16))
    ...
    def forward(self, rays_pts_emb, scales_emb=None, rotations_emb=None, opacity = None, time_emb=None, t_vertices=None, face_vertices=None, pose_params=None):
        if time_emb is None:
            return self.forward_static(rays_pts_emb[:,:3])
        else:
            return self.forward_dynamic(rays_pts_emb, scales_emb, rotations_emb, opacity, time_emb, t_vertices, face_vertices, pose_params)
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, time_emb, t_vertices, face_vertices, pose_params):
...
class deform_network(nn.Module):
    ...
    def forward(self, point, scales=None, rotations=None, opacity=None, times_sel=None, t_vertices=None, face_vertices=None, pose_params=None, item=False):
        if times_sel is not None:
            return self.forward_dynamic(point, scales, rotations, opacity, times_sel, t_vertices, face_vertices, pose_params)
        else:
    ...
    def forward_static(self, points):
        ...
        return points
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, times_sel=None, t_vertices=None, face_vertices=None, pose_params=None):

        means3D, scales, rotations, opacity = self.deformation_net( point,
                                                  scales,
                                                rotations,
                                                opacity,
                                                times_sel, t_vertices, face_vertices, pose_params)
